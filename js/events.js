...
function renderEvents(filter = "all") {
    ...
    if (!grid) return;
    const list = filter === "upcoming" ? eventsData.filter(e => e.upcoming)
            : filter === "all"      ? eventsData
            :                        eventsData.filter(e => e.region === filter);
...
function switchTab(tab) {
    document.querySelectorAll(".ev-tab").forEach(b => b.classList.toggle("active", b.dataset.tab === tab));
    document.querySelectorAll(".ev-section").forEach(s => s.classList.remove("active-section"));
    document.getElementById(tab === "events" ? "events-section" : "guides-section").classList.add("active-section");
}
...
document.addEventListener("DOMContentLoaded", () => { renderEvents(); renderGuides(); });